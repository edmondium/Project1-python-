#from scipy import *
import cython #import weave #import scipy.weave as weave
from pylab import *
import sys #, symeig
import excor
...
from numerovgen import NumerovGen
from argums import compute_argums
from legendre import compute_legendre
from ho import compute_HO
from root_mu import rootChemicalPotential
from compute_mt import ComputeMTDensity
from fccLattice import FccLattice
...
def ComputeEigensystem(k, Km, Olap_I, Enu, logDer, RMuffinTin, Vol, VKSi=0):
    ...
    weave.inline(codeArg, ['Km', 'k', 'qv', 'qvs', 'argums'],
                 type_converters=weave.converters.blitz, compiler = 'gcc')'''
    qv, qvs, argums = compute_argums(Km, k)
    ...
    weave.inline(codeLegendre, ['argums', 'lmax', 'Leg'],
                 type_converters=weave.converters.blitz, compiler = 'gcc')'''
    Leg = compute_legendre(argums, lmax)
    ...
    Olap, Ham, WK0, WK1, WK2 = compute_HO(Olap_I, qvs, Leg, PP, a_lk, b_lk, Enu, VKSi, RMuffinTin, Vol)
...
def Atom_charge(Z, core, mix=0.3, RmaxAtom=10., Natom=3001, precision=1e-5, Nitt=100):
   ...
   XC = excor.ExchangeCorrelation(5) #my research
...
if __name__ == '__main__':
    ...
    # Exchange correlations class; VWN seems to be the best (look http://physics.nist.gov/PhysRefData/DFTdata/Tables/ptable.html)    
    ...
    for itt in range(Nitt):  # self-consistent loop
        ...
        w0 = np.array(w0)   # shape (nk, nlmax, nbands)
        w1 = np.array(w1)
        w2 = np.array(w2)
