...
#   define mtrafo_r    mtrafo
#   define mtrafo_pw_r mtrafo_pw
# endif 
...
#   define Pcycle(indx,n,l)     if(pcycle(indx,n,0d0,l)) cycle
#   define PcycleM(indx,n,xm,l) if(pcycle(indx,n,xm,l))  cycle
...
/*  Using Ninit for initializing an array (e.g., array=0) obviously leads to memory doubling. */
#   define Nallocate0(arr,dim) call node_allocate(win_ arr,ptr,int(storage_size(arr)/8),dim);call c_f_pointer(ptr,arr,dim);Ocall checkmem('#'//STRINGIFY(arr),1d0*sizeof(arr));Nfence(arr);ifO arr=0;Nfence(arr)
#   define Nallocate(arr,dim)  call node_allocate(win_ arr,ptr,int(storage_size(arr)/8),dim);call c_f_pointer(ptr,arr,dim);Ocall checkmem('#'//STRINGIFY(arr),1d0*sizeof(arr));Nfence(arr)
#   define Ndeallocate(arr) Ocall checkmem('#'//STRINGIFY(arr),-1d0*sizeof(arr));call mpi_win_free(win_ arr,Merr);nullify(arr)
