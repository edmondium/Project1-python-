...
c
      ...
      if(typ<=2) then ! self-energy
        ...
        else                ; nb = size(block,2)
        endif
        if(nb<=0) Error('Number of blocks not positive. (bug?)'//FN)
        allocate(undef(size(block,2)),iarr(size(block,1)))
        undef = .true.
        do ib = 1,nb
          iarr = 0
     ...
     &            Error('JOB k point ('//Chn(kpt(:,k),',')//') differs from stored one ('//Chn(kvec,',')//').'//FN)
            ...
            if(n/=count(block(:,ib)/=0).or.k/=job1%kpt(block(1,ib)).or.s/=job1%spin(block(1,ib))
...
# define FN ' File: '//trim(fname)//trim(sfx)
      ...
      if(typ<=2) then ! Write selfx(c)
        write(iunit) size(block,2),size(block,1),block
        k = 0
...
# define ERROR(arg) then; error = arg; return; endif
      ...
      if(lfull) then
        ...
        else if(abs(checksum1-checksum)>1d-12) then
