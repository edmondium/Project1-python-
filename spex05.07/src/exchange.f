...
# include "restype.h"      
      ...
      MCOMPLEX_dp              :: cprod0(maxbasm)
      real_dp                  :: kvec(3),mom(3),rdum
...
c         Calculate wave-function products
          call cpu_time(time1)
          allocate ( cprod(maxbasm,bandi:bandf,size(band),Mcol1(nkpt1)) )
...
c            call matrixtrafo(coul2,coul1,ikpt,sym(symkpt(ikpt))%inv,.true.)
            ...
            do i = bandi,bandf
              ...
              do j = 1,size(band)
                call mtrafo_r(cprod0,cprod(:,i,j,ikpt1),nbasm(ikpt),1,kptp(ikpt),-symkpt(ikpt),1,.false.)
                if(fullpw) cprod0(nbasp+1:nbasm(ikpt)) = matvec(olap(:n,kptp(ikpt)),cprod0(nbasp+1:nbasm(ikpt)))
                cprod(:,i,j,ikpt1) = cprod0
              enddo
            enddo
            ...
            do iband = bandi,bandf
              ...
              do j = 1,size(band)
                ...
                cprod1 = matvec ( coulomb(:nn,kptp(ikpt)) , cprod(:n,iband,j,ikpt1) )
                do i = 1,j
                  ! scalar products: conjg(cprod1) * cprod(i)
                  cdum = dotprod ( cprod1 , cprod(:n,iband,i,ikpt1) ) *
...
c       (B) Analytic integration over 1/k**2  (diagonal elements)                 (-> "pole" in the output)
     ...
     &                   ( ene(iband,ikptq,ispin) - ene(iband2,ikptq,ispin) ) * vol
          ...
          if(ologdiv) then
            ...
            mat(j,j) = mat(j,j) + 4*pi/vol * divergence / nsym1 * logdiv(ene(iband2,ikptq,ispin)-efermi,mom)
