     ...
     &                                                   core2(:,nc,lc,itype,ispin) * bas2(:,n,l,itype,ispin) ) ,itype)
                ...
                do ieq = 1,neq(itype)
                  ...
                  do mc = -lc,lc
                    ...
                    enddo
                    enddo ! SOC loop
                    if(llm.ne.sum([((2*i+1)*nindxm(i,itype),i=0,lcutm(itype))])) Bug('Count error.')
...
c     Loop over bands
      ...
      do iblock = 1,size(block,2)
        ...
        if(oselfc==4) then
          ...
          if(job1%full) Bug('FULL matrix calculations impossible with this self-energy type.')
          if(eneq<=efermi) then ; freqr1 = eneq - efermi - freqr
          else                  ; freqr1 = eneq - efermi + freqr
          endif
        endif
        ...
        if(ologdiv) then
          do ib = 1,size(block,1)
            if(block(ib,iblock)/=0) then
              ...
              do ifreqr = 1,nfreqr
                w       = freqr1(ifreqr)
                logdiv2 = logdiv(eneq-w,mom)
                if( w<0  .and. logdiv1*(1-logdiv2)>1d-8 .or.
     &              w>=0 .and. logdiv2*(1-logdiv1)>1d-8 ) then
                  f1 = min(f1,w-eneq)
                  f2 = max(f2,w-eneq)
                endif
              enddo
