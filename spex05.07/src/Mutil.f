...
c      
      ...
      select case(mode)
        case(0) ! initialize: Non-root process opens file where unit-6 output will be stored
          if(lfile) then
            ...
            open(6,file='spex.mwrite.'//Chr(uin)//'r'//Chr(Mrank0)//'q'//Chr(i))
          endif
      ...
      iunit = fopen('spex.mwrite.'//Chr(uin)//'r'//Chr(rank)//'q'//Chr(itag),status='old')
      do
...
c     Function pcycle(i,n,...) returns .true. if current process should work on ith loop (with a total of n loops).
c
...
c
c      
c     Typical usage: do i = 1,n ; if(pcycle(i,n,0d0,.true.) cycle ! this if clause can be abbreviated with macro Pcycle of cppmacro.h
c                      beginSplit(i)                              ! optional communicator splitting (required for ScaLAPACK)
c                      ...
c                      endSplit
c                    enddo

      function pcycle(indx,n,xmem,lscalapack)
...
c     dim0(:ndim)       : Absolute values: Dimensions of array, e.g., [20,8,6] for array(20,8,6).
c                         Sign: select kind of output (see bounds).
...
c                         If bounds(:2,:)=[1,0], current Mrank is taken idle for this step.
      ...
      integer              :: rnk,rank,siz(ndim)
      integer              :: i,nidle,rank1
      ...
      if(Mrank>=i) then
        ...
        do rank = 0,Msize-1
          do i = 1,ndim
            rnk         = mod( rank/product(siz(:i-1)) , siz(i) )
            bounds(1,i) =  1+rnk *dim(i)/siz(i)
            bounds(2,i) = (1+rnk)*dim(i)/siz(i)          
          enddo
          if(all(bounds(2,:)>=bounds(1,:))) then ; if(rank1==-1) rank1 = rank ! rank1 = lowest rank with non-empty work package
          else                                   ; nidle = nidle + 1          ! count idle processes
          endif
        enddo
        rank = Mrank
        if     (rank1==-1) then ; Bug('All MPI work packages are empty.')
        else if(rank1>  0) then ! avoid that root gets an empty work package (redefine rank)
          if     (Mrank==0)     then ; rank = rank1
          else if(Mrank==rank1) then ; rank = 0
          endif
        endif
        do i = 1,ndim
          rnk         = mod( rank/product(siz(:i-1)) , siz(i) )
...
c
c     Root creates a file "spex.task.UIN" with MPI I/O containing the numbers 1,2,3,...,number+size.
...
c
      ...
      integer              :: i
      if(Mrank==0) then
        call mpi_file_open(mpi_comm_self,'spex.task.'//Chr(uin),ior(mpi_mode_wronly,mpi_mode_create),mpi_info_null,Mfile,Merr)
        do i = 1,number+size
          call mpi_file_write(Mfile,i,1,mpi_integer,Mstat,Merr)
        enddo
        call mpi_file_close(Mfile,Merr)
      endif
      call mpi_file_open(Mcomm,'spex.task.'//Chr(uin),ior(mpi_mode_rdonly,mpi_mode_delete_on_close),mpi_info_null,Mfile,Merr)
      end

c
c     Reads the next task from the list in "spex.task.UIN".
c
c     indx  : index of next task (output), index of previous task (input, only required if min>0)
