...
# endif      
      ...
      real_dp,      allocatable :: eig(:),fwancent(:,:,:)
      real_dp                   :: kvec(3),kvec0(3),dev,lat1(3,3)
      real_dp                   :: rdum,dist0,dist1,dist(ncent)
...
# endif
      ...
      if(ldum) then
        ...
        else if(i>-1) then ; Error('File format of "spex.uwan" too old. Cannot be read.')
        else if(i<-1) then ; Error('File format of "spex.uwan" too new. Cannot be read.')
        endif
     ...
     &    Error('Checksum failure. Current wave functions inconsistent with stored Wannier coefficients. Remove spex.uwan.')
        ...
        if(any([nwan,wanbandi,wanbandf]/=[i1,i2,i3])) then
          ...
          write(6,'(A)')  '- Number of Wannier functions: '//Chr(i1)           //', formerly '//Chr(nwan)
          write(6,'(A/)') '- Wannier band window: '//Chn((/i2,i3/),'-')//', formerly '//Chn((/wanbandi,wanbandf/),'-')
...
# endif
     ...
     &                                        + dot_product(cpw_(:ngpt(koff+ikpt),i1,ikpt,2),cpw2)
            ...
            if(iand(restart,W_kolap)/=0) then
              ...
              write(iunit) wanbandi,wanbandf,nntot,nkpt,ispin,off,kptadd,checksum
              write(iunit) kolap
              call fclose(iunit)
     ...
     &          Error('Checksum failure. Current wave functions inconsistent with stored Wannier coefficients. Remove spex.uwan.')
              ...
              Warn('Did not find checksum in spex.uwan. Cannot check consistency of wave functions.')
              backspace(iunit)
...
# endif
      ...
      if(iand(restart,W_uwan)/=0) then
        ...
        write(iunit) -1
        write(iunit) nkpt,nspin1,kptadd,checksum
...
# endif
 ...
 10   deallocate ( lwan,lmwan,centwan,eulerwan )
      ...
      if(l_inter) call wannier_interpolation('0',(1d0,0d0)*ene(wanbandi:wanbandf,:nkpti,:),0)
...
c --------------
      ...
      logical             :: lexist,lentry(4)
      real_dp, parameter  :: minproj = 0.5d0 ! each frozen state must have a total projection into the Wannier basis of minproj or higher
      real_dp             :: efrozen1,r
      ...
      if(l_frozen) then
        if(efrozen(ispin)==huge(0d0)) then
          ...
          do k = 1,nkpti
            do i = wanbandi,wanbandf
              if(sum(abs(uwan(i,:,k,ispin))**2)<minproj) efrozen1 = min( efrozen1 , ene(i,k,ispin) - 1d-6 )
            enddo
          enddo
          ...
          if(r==efrozen1) then
            ...
            write(6,'(A)') 'Highest possible dis_froz_max with this number of Wannier functions: '//Chf(r*hartree,'F10.6')//' eV'
          endif
        else
...
c end interface
 ...
 1    ind    = index(label,'/')
      ...
      if(ind==0) then
        ...
        angl = ch2r(label(ind+1:ind+ind1-1),'ORBITALS/BANDINFO') ; if(define) eulerwan(1,nwan+1) = angl        
        ind  = ind + ind1
        ind1 = index(label(ind+1:),'/')                          ; if(ind1==0) Error('Incorrect definition of Euler angles.')
        angl = ch2r(label(ind+1:ind+ind1-1),'ORBITALS/BANDINFO') ; if(define) eulerwan(2,nwan+1) = angl        
        ind  = ind + ind1        
        angl = ch2r(label(ind+1:),          'ORBITALS/BANDINFO') ; if(define) eulerwan(3,nwan+1) = angl        
        ind  = index(label,'/') - 1
      endif
...
c     Returns ilabel-th orbital label in "label" and atom index in "ic" from orbitals definition charr(:ncharr).
      ...
      do i = 1,ncharr
        l  = len_trim(charr(i))
        ic = ic + 1
        if(ic>ncent)                                           Error('Atom count error in ORBITALS or BANDINFO.')
        if(all( charr(i)(1:1)//charr(i)(l:l) /= ['[]','()'] )) Error('Expected (...) or [...] in line ORBITALS or BANDINFO.')                
        if(charr(i)(1:1)=='[') then
          itype = atype(ic)
...
# include "interface/band_info.inc"
      ...
      call getkey(inp,'INTERPOL', qfile,  section='WANNIER', default=' ', status=i) ; if(i==1) qfile = ' '
