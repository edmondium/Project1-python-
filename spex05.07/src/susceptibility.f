...
# ifdef __GFORTRAN__
      MCOMPLEX_dp, intent(out), target :: susceph(dim*(dim+1)/2,nfrqh) ! otherwise gfortran 4.9 complains
...
# endif
      ...
      complex_dp               :: carr(nbasm(ikpt)) NoInvC(cdum)
      MCOMPLEX_dp              :: mdum1,mdum2

      real_dp,     allocatable :: wghtSr(:,:),wghtSi(:,:),wtetra(:,:),re(:),im(:),sf(:),reS(:,:) NoInvC(imS(:,:))
      ...
      integer                  :: kpt1(nkpt),nkpt1,kpt1p(nkpt),nkpts(nkpt),sym1(nsym),nsym1
      integer                  :: kpt1_(nkpt),nkpt1_,nkpts_(nkpt)
...
# include "interface/getfrqs.inc"
      ...
      call timer_start('routine susceptibility')
      Rcall timer_start('preparations')
...
c      if(any(real(frqc)<0)) Error('frequency has negative real part.')
      ...
      write(6,'(A,F10.1," MB")') 'Size of array suscep:',(MBYTES*dim*(dim+1)/2*nfrqh+16d0*dim**2*nfrqc)/megabyte
...
c     Prepare irreducible representations inside eigenspaces of coulomb (->cblock,irrep,dirrep,nirrep)
      ...
      if(nsym1>1.and.trafo) then
        Rcall timer_stop('preparations')
        call timer_start('irreps')
...
# endif
        deallocate ( nindex )
        call timer_stop('irreps',time=time1)
        Rbegin
        write(6,'(A,F13.1,A)') 'Non-zero elements:',count(sirrep>=sirrep_threshold)*100d0/dim**2,' %   ('//Chr(time1)//')'
...
c      do i = 2,nfrqc
c        if(real(frqc(i))<real(frqc(i-1)))
c     &    Error('Frequencies must be ordered according to increasing real parts.')
c      enddo
...
c
c     Allocate arrays reS and imS already here because their "cost" should be taken into account in MEM separation
      if(nfrqs/=0) then ; ifrq = nfrqs
      else              ; ifrq = nfrqh + nfrqc
      endif
      Allocate_( reS,(ifrq,dim) )
      NoInv( Allocate_ ( imS,(ifrq,dim) ) )
...
# ifdef LOAD
      ...
      memo = ( maxmem - mem ) Mpi(/Nsize) ! memory available for current rank
      if( memo < (memc*(no+nu)+memp*no*nu)*nk ) then ! we need MEM work packages
c        if( memo >= (memc*(no+nu)+memp*no*nu) ) then ! kpoint division sufficient
c          nkmax = int( memo / (memc*(no+nu)+memp*no*nu) )
c        else                                         ! ... insufficient
c          nkmax = 1
          rdum  = 2*memc/(3*memp)
...
# else
      ...
      ! (a) kpt1 decomposition (MPI)
# if defined(MPI) && !defined(LOAD)
      if(first_term) then
        d             = ggt(Msize,nkpt1)
        nkpt1         = nkpt1 / d
        msiz          = Msize / d       ! defined for Mdistribute
        rank          = mod(Mrank,msiz) !
        kpt1 (:nkpt1) = kpt1 (1+Mrank/msiz*nkpt1:(Mrank/msiz+1)*nkpt1)
        nkpts(:nkpt1) = nkpts(1+Mrank/msiz*nkpt1:(Mrank/msiz+1)*nkpt1)
        Rif(d>1) write(6,'(''k-point packets:'',12X,A)') trim(chr(d))//' ('//trim(chr(nkpt1))//')'
      endif
# endif

      ! (b) MPI work packages (MPI)
      no1 = 1
      no2 = bando
      nu1 = 1
      if(nfrqs==0) then ; nu2 = maxband
      ...
      if(oibc==0) nu2 = nu2 - bandu + 1
      Mpi( call Mdistribute(bando,nu2-nu1+1) ) ! distributes occ*unocc pairs to processes (->no1,no2,nu1,nu2)
      if(oibc==0) then
        nu1 = nu1 + bandu - 1
        nu2 = nu2 + bandu - 1
      endif
      ...
      ! memory factor for wave functions
      if(ikpt==1) then ; memc =    (16d0*maxlmindx+MBYTES*maxgpt)*nkpt1*nspin3
      else             ; memc = max(16d0*maxlmindx,MBYTES*maxgpt)*nkpt1*nspin3
      endif
      NoLoad( memc = 0 )
      ! memory factor for wave-function products, weights (wtetra), and index (if PREORDER undefined)
# ifdef PREORDER
      memp = (MBYTES*maxbasm+8*nfrqs        )*nkpt1
# else
      memp = (MBYTES*maxbasm+8*nfrqs+4*nfrqs)*nkpt1
# endif

      ! (c) MEM work packages
      Nfence(mem)
      memo  = ( maxmem - mem ) Mpi(/Nsize)
      rdum  = 2*memc/(3*memp)
      nomax = min ( max ( int ( sqrt(rdum**2+memo/(3*memp)) - rdum ), 1 ) , no ) ! from  no*nu*memp + (no+nu)*memc = memo  with nu = 3*no
      d1    = (no-1)/nomax + 1
      nomax = (no+d1-1)/d1
      numax = (memo-nomax*memc) / (memc+nomax*memp Mpi(+8*nfrqs*maxval(kpt1p)) ) ! MPI: added for auxiliary Mwintgr5 array
      d2    = (nu-1)/numax + 1
      numax = (nu+d2-1)/d2
      Rif(first_term) then
        if(nomax/=no.or.numax/=nu) then
          write(6,'(A,10X,A)') 'MEM work packages:',trim(chr(d1))//'*'//trim(chr(d2))//
     &                                     ' ('//trim(chr(nomax))//'*'//trim(chr(numax))//')'
        endif
        write(6,'(A,F13.1,A)') 'Auxiliary storage:',memo/megabyte,' MB'
      endif
      if(nomax*numax==0) Error('Maximal allowed storage exceeded. Increase MEM or reduce number of frequencies!')
      Nfence(mem)
...
c
c     Loop over states in packets of no and nu bands
      line = ' '
...
# ifdef old_distrib
      ...
      do while(bu1<=nu2) ; bu2 = min(bu1+numax-1,nu2) ; nu = bu2 - bu1 + 1
        n = len_trim(chr(no)) + len_trim(chr(nu)) + len_trim(chr(bo1)) + len_trim(chr(bu1))
# ifdef MPI
        write(OUTP,'(I'//chr(18-n)//',A,1X,A'NOA) Mrank/msiz,'/'//trim(chr(bo1))//'/'//trim(chr(bu1)),
     &                                            trim(chr(no))//'*'//trim(chr(nu))
# else
        write(OUTP,'('//chr(19-n)//'X,A,1X,A'NOA) trim(chr(bo1))//'/'//trim(chr(bu1)),trim(chr(no))//'*'//trim(chr(nu))
# endif
...
# else
      ...
      do while(bu1<=nu2) ; bu2 = min(bu1+numax-1,nu2) ; nu = bu2 - bu1 + 1
        ...
        nkpt1         = nk
        kpt1(:nkpt1)  = [ (kpt1_(k), k=bk1,bk2) ]
        nkpts(:nkpt1) = [ (nkpts_(k),k=bk1,bk2) ]
# endif
...
c                    k,I    qn      q+kn'
        ...
        if(dble(nu)*no*nkpt1>huge(0)) Bug('Integer overflow for cprod. Please report.')
        Allocate_ ( cprod,(nu*no*nkpt1,nbasm(ikpt)) )

        Load( Rcall timer_start('read_wavef1') )
        Load( if(ikpt==1) call read_wavef1(bo1,bo2,bu1,bu2,kpt1,nkpt1,ikpt,ispin1,ispin2,.true.,.true.)  ) ! k=0  : we need cmt,cpw for later
        Load( if(ikpt/=1) call read_wavef1(bo1,bo2,bu1,bu2,kpt1,nkpt1,ikpt,ispin1,ispin2,.true.,.false.) ) ! k/=0 : only read cmt
        Load( Rcall timer_stop('read_wavef1') )

        beginSplit(bo1*nkpt+kpt1(1))
        call timer_start('wavefproducts_mt')
        call wavefproducts2_mt(cprod,            kpt1,nkpt1,ikpt,ispin1,ispin2, BANDDEF )
        call timer_stop('wavefproducts_mt',time=time1)
        write(OUTP,'(F10.2'NOA) time1 ; MpiR_WRITE
        endSplit

        Load( Rcall timer_start('read_wavef1') )
        Load( if(ikpt/=1) deallocate(cmt) )
        Load( if(ikpt/=1) call read_wavef1(bo1,bo2,bu1,bu2,kpt1,nkpt1,ikpt,ispin1,ispin2,.false.,.true.) ) ! k/=0 : only read cpw
        Load( Rcall timer_stop('read_wavef1') )
        
        beginSplit(bo1*nkpt+kpt1(1))
        call timer_start('wavefproducts_pw')
        call wavefproducts2_pw(cprod(:,nbasp+1:),kpt1,nkpt1,ikpt,ispin1,ispin2, BANDDEF )
        call timer_stop('wavefproducts_pw',time=time1)
...
# define STEP (dim/4)
        if(trafo) then
          ...
          NoInv( cprod = conjg(cprod) )
          call timer_stop('trafo',time=time1)
          write(OUTP,'(F8.2'NOA) time1 ; MpiR_WRITE
# ifdef Test_cprod
...
c       transition metals (e.g., Ni) at a small (or zero) k vector.
        if(ikpt==1.and.ispin1==ispin2) then
          allocate ( dcprod (nu*no*nkpt1,3) )
          call timer_start('momentum')
          call momentum_matrix( dcprod,kpt1,nkpt1,ispin1,ispin2 , BANDDEF , .false. MpiC(.false.) LoadC(.false.) )
          call timer_stop('momentum',time=time1)
          write(OUTP,'(F8.2'NOA) time1 ; MpiR_WRITE
# ifdef Test_cprod
          allocate(ctrafo1(nu*no*nkpt1,1))
        ...
        if(multdiff_/=0.and.trafo) then
          ...
          do i = 1,nkpt1
            ...
            do j = bo1,bo2
              do k = bu1,bu2
                i1          = i1 + 1
                rdum        = ene(k,ikpt2,ispin2)-ene(j,ikpt1,ispin1)
...
c       Loop over spectral-function frequencies and calculate BZ integration weights
        if(nfrqs/=0) then
          call timer_start('weights')
          Allocate_ ( wtetra,(n,nfrqs) )
          call getkpt1p(kpt1p,kpt1,nkpt1,sym1,nsym1,nkpts,.false.)
          wtetra = 0
          if(gauss(1)/=0) then
            call gauss4_init      (wtetra,frqs,nfrqs,ikpt,kpt1,kpt1p,nkpt1,1,ispin1,ispin2,bo1,bo2,bu1,bu2)
          else
            call tetrahedron5_init(wtetra,frqs,nfrqs,ikpt,kpt1,kpt1p,nkpt1,1,ispin1,ispin2,bo1,bo2,bu1,bu2,bo1,bo2,bu1,bu2)
          endif
          wtetra = wtetra / nsym1
          call timer_stop('weights',time=time1)
          write(OUTP,'(F8.2'NOA) time1 ; MpiR_WRITE
        else
...
c       NO PREORDER: Define array index of nonzero weights wtetra.
        if(nfrqs/=0) then
          Timing( call timer_start('  suscep-prep') )
...
# else
          ...
          enddo
# endif
          Timing( call timer_stop('  suscep-prep') )
        endif
...
c       ----------------------------------------
        ...
        beginSplit(1) ! split off active processes (to avoid deadlock)
...
# endif
                enddo
                call spectral_function(reS(:,k1) NoInvC(imS(:,k1)) ,l)
              enddo
              Mpi( call Msum(reS(:,:k2)) ; NoInv( call Msum(imS(:,:k2)) ) )
              if(nfrqs==0) then ! distribute (HILBERT NONE)
                do ifrq = Mrange1(nfrqh)
                  ij = (k2-1)*k2/2
                  do k1 = 1,k2
                    ij = ij + 1
                    headh(ij,ifrq) = headh(ij,ifrq) + reS(ifrq,k1) NoInv( + imS(ifrq,k1) )
                  enddo
                enddo
                do ifrq = Mrange1(nfrqc)
                  do k1 = 1,k2
                    headc(k1,k2,ifrq) = headc(k1,k2,ifrq) + reS(nfrqh+ifrq,k1) NoInv( + img * imS(nfrqh+ifrq,k1) )
                    if(k1/=k2)
     &              headc(k2,k1,ifrq) = headc(k2,k1,ifrq) + reS(nfrqh+ifrq,k1) NoInv( - img * imS(nfrqh+ifrq,k1) )
                  enddo
                enddo
              else ! HILBERT trafo
                do ifrq = Mrange1(nfrqh)
                  ...
                  do k1 = 1,k2
                    ij = ij + 1
                    mdum1 = ddot(nfrqs,reS(1,k1),1,wghtSr(1,ifrq),1) NoInv(+img*ddot(nfrqs,imS(1,k1),1,wghtSr(1,ifrq),1))
                    headh(ij,ifrq) = headh(ij,ifrq) + mdum1
                  enddo
                enddo
                do ifrq = Mrange1(nfrqc)
                  do k1 = 1,k2
                    mdum1= ddot(nfrqs,reS(1,k1),1,wghtSr(1,nfrqh+ifrq),1) NoInv(+img*ddot(nfrqs,imS(1,k1),1,wghtSr(1,nfrqh+ifrq),1))
                    mdum2= ddot(nfrqs,reS(1,k1),1,wghtSi(1,nfrqh+ifrq),1) NoInv(+img*ddot(nfrqs,imS(1,k1),1,wghtSi(1,nfrqh+ifrq),1))
                    headc(k1,k2,ifrq) = headc(k1,k2,ifrq) + mdum1 + img*mdum2
                    if(k1/=k2)
     &              headc(k2,k1,ifrq) = headc(k2,k1,ifrq) + MCONJG(mdum1) + img*MCONJG(mdum2)
                  enddo
                enddo
              endif
            enddo
...
# endif
                enddo
                call spectral_function(reS(1,i) NoInvC(imS(1,i)) ,l)
              enddo
              Mpi( call Msum(reS) ; NoInv( call Msum(imS) ) )
              if(nfrqs==0) then ! distribute (HILBERT NONE)
                do ifrq = Mrange1(nfrqh)
                  do i = 1,dim
                    wingh(k2,i,ifrq) = wingh(k2,i,ifrq) + reS(ifrq,i) NoInv( + imS(ifrq,i) )
                  enddo
                enddo
                do ifrq = Mrange1(nfrqc)
                  do i = 1,dim
                    wingc(k2,i,1,ifrq) = wingc(k2,i,1,ifrq) + reS(nfrqh+ifrq,i) NoInv( + img * imS(nfrqh+ifrq,i) )
                    wingc(k2,i,2,ifrq) = wingc(k2,i,2,ifrq) + reS(nfrqh+ifrq,i) NoInv( - img * imS(nfrqh+ifrq,i) )
                  enddo
                enddo
              else ! HILBERT trafo
                do ifrq = Mrange1(nfrqh)
                  do i = 1,dim
                    mdum1 = ddot(nfrqs,reS(1,i),1,wghtSr(1,ifrq),1) NoInv(+img*ddot(nfrqs,imS(1,i),1,wghtSr(1,ifrq),1))
                    wingh(k2,i,ifrq) = wingh(k2,i,ifrq) + mdum1
                  enddo
                enddo
                do ifrq = Mrange1(nfrqc)
                  do i = 1,dim
                    mdum1 = ddot(nfrqs,reS(1,i),1,wghtSr(1,nfrqh+ifrq),1) NoInv(+img*ddot(nfrqs,imS(1,i),1,wghtSr(1,nfrqh+ifrq),1))
                    mdum2 = ddot(nfrqs,reS(1,i),1,wghtSi(1,nfrqh+ifrq),1) NoInv(+img*ddot(nfrqs,imS(1,i),1,wghtSi(1,nfrqh+ifrq),1))
                    wingc(k2,i,1,ifrq) = wingc(k2,i,1,ifrq) + mdum1 + img*mdum2
                    wingc(k2,i,2,ifrq) = wingc(k2,i,2,ifrq) + MCONJG(mdum1) + img*MCONJG(mdum2)
                  enddo
                enddo
              endif
            enddo
...
c       (B) Body
c        Mpi( call mpi_barrier(Ncomm,Merr) )
c# warning barrier added for testing
        ...
        do j = 1,dim
          ...
          if(trafo.and.multdiff_==2) then ! multdiff_ = 2
            if(ll==0) i1 = 2              !   only body,          loop i = 2,j
            if(ll> 0) i2 = 1              !   only head or wings, loop i = 1,1
          endif
          do i = i1,i2
            if(sirrep(i,j)<sirrep_threshold) then ; Mpi( reS(:,i) = 0 ; NoInv( imS(:,i) = 0 ; ) ) cycle ; endif ! skip zero elements (set array to zero for Msum)
            if(trafo.and.multdiff_==1) l = count([i,j]==1)
            Timing( call timer_start('  suscep-cprods:'//Chr(n)) )
            do k = 1,n
# ifdef INV
...
# endif
            enddo
            Timing( call timer_stop('  suscep-cprods:'//Chr(n)) )
            call spectral_function(reS(1,i) NoInvC(imS(1,i)) ,l)
          enddo
          Timing( call timer_start('  suscep-distrib') )
          Mpi( call Msum(reS(:,i1:i2)) ; NoInv( call Msum(imS(:,i1:i2)) ) )
          if(nfrqs==0) then ! distribute (HILBERT NONE)
            do ifrq = Mrange1(nfrqh)
              ij = (j-1)*j/2 + i1 - 1
              do i = i1,i2
                ij = ij + 1 ; if(sirrep(i,j)<sirrep_threshold) cycle
                susceph(ij,ifrq) = susceph(ij,ifrq) + reS(ifrq,i) NoInv( + img * imS(ifrq,i) )
              enddo
            enddo
            do ifrq = Mrange1(nfrqc)
              do i = i1,i2 ; if(sirrep(i,j)<sirrep_threshold) cycle
                suscepc(i,j,ifrq) = suscepc(i,j,ifrq) + reS(nfrqh+ifrq,i) NoInv( + img * imS(nfrqh+ifrq,i) )
                if(i/=j)
     &          suscepc(j,i,ifrq) = suscepc(j,i,ifrq) + reS(nfrqh+ifrq,i) NoInv( - img * imS(nfrqh+ifrq,i) )
              enddo
            enddo
          else  ! HILBERT trafo
            do ifrq = Mrange1(nfrqh)
              ...
              do i = i1,i2
                ij = ij + 1 ; if(sirrep(i,j)<sirrep_threshold) cycle
                mdum1 = ddot(nfrqs,reS(1,i),1,wghtSr(1,ifrq),1) NoInv( + img * ddot(nfrqs,imS(1,i),1,wghtSr(1,ifrq),1) )
                susceph(ij,ifrq) = susceph(ij,ifrq) + mdum1
              enddo
            enddo
            do ifrq = Mrange1(nfrqc)
              do i = i1,i2 ; if(sirrep(i,j)<sirrep_threshold) cycle
                mdum1 = ddot(nfrqs,reS(1,i),1,wghtSr(1,nfrqh+ifrq),1) NoInv( + img * ddot(nfrqs,imS(1,i),1,wghtSr(1,nfrqh+ifrq),1) )
                mdum2 = ddot(nfrqs,reS(1,i),1,wghtSi(1,nfrqh+ifrq),1) NoInv( + img * ddot(nfrqs,imS(1,i),1,wghtSi(1,nfrqh+ifrq),1) )
                suscepc(i,j,ifrq) = suscepc(i,j,ifrq) + mdum1 + img*mdum2
                if(i/=j)
     &          suscepc(j,i,ifrq) = suscepc(j,i,ifrq) + MCONJG(mdum1) + img*MCONJG(mdum2)
              enddo
            enddo
          endif
          Timing( call timer_stop('  suscep-distrib') )
        enddo
        ...
        call timer_stop('suscep',time=time1)
        write(OUTP,'(F8.2)') time1 ; MpiR_WRITE

        endSplit ! end split active processes
...
c     Inactive processes "simulate" a packet to avoid deadlocks      
 5    do d = 1,nskip
        do i = 1,3
          beginSplit(0) ! inactive processes (avoid deadlock)
          endSplit
        enddo
...
c     Calculate Drude term (plasma frequency)
      if(ikpt==1.and.metal.and.spin<=2) then
        ...
        Rcall timer_stop('plasma',time=time1)
...
c     Add contribution of symmetry-equivalent q points
      if(nsym1>1) then
        ...
        Rcall timer_start('symmetry-equiv')
c       (A) Body
        if(trafo) then
...
# ifdef OLD_ALGO
            if(ifrq<=nfrqh) then ; susceph(:,ifrq)         = packmat(suscep1,ltest=.false.)
...
c       (B) Head and wings
        ...
        Rcall timer_stop('symmetry-equiv',time=time1)
...
c     Add core contribution
      if(any(cores)) then
        Rcall timer_start('add_core')
        call add_core
        Rcall timer_stop('add_core')
      endif
...
# endif
      ...
      call timer_stop('routine susceptibility')
      call timer_print(['routine susceptibility'],since=.true.,title='TIMING SUSCEPTIBILITY',keep=.true.,prnt=otimer andR )
...
c     Calculate spectral function at frequencies frqs (->reS+img*imS)
      subroutine spectral_function(reS NoInvC(imS) ,mm)
      implicit none
      integer, intent(in) :: mm
      real_dp             :: reS(*) NoInvC(imS(*))
...
# endif
      ...
      if(nfrqs==0) then
        do ifrq = 1,nfrqh+nfrqc
                 reS(ifrq) = ddot(n,re,1,wghtSr(1,ifrq),1)
          NoInv( imS(ifrq) = ddot(n,im,1,wghtSr(1,ifrq),1) )
        enddo
        return
      endif

      ! Calculate spectral function for HILBERT
      Timing( call timer_start('  suscep-specf') )
# ifdef PREORDER
      do ifrqs = 1,nfrqs
...
# endif
      ...
      Timing( call timer_stop('  suscep-specf') )
...
c     IBC : Calculate double-counting term (->ibc_suscep1)
      ...
      implicit none
      integer :: i,j,k,i1,i2
      call timer_start('IBC double_counting')
# ifdef FIBC
...
# endif
      call timer_stop('IBC double_counting',time=time1)
...
# ifdef FIBC
      ...
      call timer_start('IBC add_to_suscep')
#   ifdef INV
      if(trafo) then
...
# ifdef MPI
      ...
      integer             :: itype,l
      real_dp             :: overhead
      overhead = 1d0/6 * sum( [ ((neq(itype)*(2*l+1)*nindx(l,itype),l=0,lcut(itype)),itype=1,ntype) ] ) / ncent ! the prefactor 1/6 was determined experimentally
# ifdef LOAD
      if(ndim/=2) Bug('Array dimension is not two.')
