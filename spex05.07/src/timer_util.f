...
c              true  : - " - , the number of timers (ntimer) is increased by one and the index is set to ntimer
      ...
      if(present(next)) then
        if(next) then
          ntimer      = ntimer + 1
          timer_index = ntimer
          if(timer_index>maxtimer) Bug('Timer limit exceeded.')
          return
        endif
      endif
      timer_index = 0
...
c       time : walltime (kind='wall') or cputime (kind else) since timer_start (last time interval)
      subroutine timer_stop(name,time,kind)
      implicit none
      character(*),           intent(in)  :: name
      character(*), optional, intent(in)  :: kind
      ...
      if(present(time)) then
        if(kind=='wall') then ; time = real(walltime)/wallrate
        else                  ; time = cputime - timer(i)%cputime0
        endif
      endif
...
c       since = true : all timers since names(1) (size(names) must be 1) (default false)
c       prnt  = true : print information (default true)
      subroutine timer_print(names, title, keep, err, since, prnt)
      use util, only: chr      
      implicit none
      character(*),           intent(in) :: names(:)
      character(*), optional, intent(in) :: title
      integer,      optional, intent(in) :: err
      logical,      optional, intent(in) :: keep,since,prnt
      logical                            :: since1,prnt1
      character(lenname)                 :: name
      real                               :: cputime,walltime
      integer                            :: delete(size(names))
      ...
      if(present(title).and.prnt1) write(6,'(/'//chr(len_trim(title))//'A/A)') ('-',i=1,len_trim(title)),trim(title)
      l  = 0
      j1 = 1
      if(since1) then
      ...
      do j = j1,j2
        ...
        if(cputime>=0) then
          l = l + 1
          if(prnt1) write(6,'(A)') name//'CPU: '//trim(chr(cputime,'F7.2'))//          
     &                             '      Wall: '//trim(chr(walltime,'F7.2'))//'      #calls: '//trim(chr(ncall,'I3'))
        endif
      enddo
      if(l==0) Warn('Did not find a valid timer for timing information.')
      if(present(title).and.prnt1) write(6,'('//chr(len_trim(title))//'A/)') ('-',i=1,len_trim(title))
