from scipy import *
from numba import njit #import weave #import scipy.weave as weave
from pylab import *
import sys #, symeig
import excor
...
class FccLattice:
    ...
    def GenerateReciprocalVectors(self, q, CutOffK):
        ...
        Kmesh0.sort(key=functools.cmp_to_key(lambda x, y: cmp(dot(x, x), dot(y, y)))) #Kmesh0.sort(lambda x,y: cmp(dot(x,x), dot(y,y)))
        self.Km = array(Kmesh0)
        print("K-mesh size =", len(self.Km)) #print "K-mesh size=", len(self.Km)
    ...
    def ChoosePointsInFBZ(self, nkp, type=0): # Chooses the path in the 1BZ we will use
        ...
        if type==0: # Choose mesh in the 1BZ to cover the whole space - for SC calculation
            ...
            for i0 in range(nkp):
                ...
                for i1 in range(nkp):
                    ...
                    for i2 in range(nkp):
                        ...
                        kp.append(k)
            print("Number of all k-points =", len(kp)) #print "Number of all k-points=", len(kp)
            ...
            print("Number of irreducible k points is", len(self.kp)) #print "Number of irreducible k points is: ", len(self.kp)
...
def Numerov(F, dx, f0=0.0, f1=1e-3):
    ...
    Nmax = len(F)
    #dx = float(dx)
    Solution = np.zeros(Nmax, dtype=np.float64) #Solution = zeros(Nmax, dtype=float)
    Solution[0] = f0
    Solution[1] = f1
    h2 = dx*dx
    h12 = h2/12
    w0 = (1 - h12 * F[0]) * Solution[0] #weave.inline(codeNumerov, ['F', 'Nmax', 'dx', 'Solution'],
    Fx = F[1]             #type_converters=weave.converters.blitz, compiler = 'gcc')
...
def NumerovGen(F, U, dx, f0=0.0, f1=1e-3):
    ...
    Solution[0] = f0
    Solution[1] = f1
    h2 = dx * dx #weave.inline(codeNumerov, ['F', 'U', 'Nmax', 'dx', 'Solution'],
    h12 = h2 / 12.0             #type_converters=weave.converters.blitz, compiler = 'gcc')
    w0 = Solution[0] * (1 - h12 * F[0]) - h12 * U[0]
    Fx = F[1]
    Ux = U[1]
    w1 = Solution[1] * (1 - h12 * Fx) - h12 * Ux
    Phi = Solution[1]
    for i in range(2, Nmax):
...
def CRHS(E, l, R, Veff):
    ...
    """'''
    N = len(R)
    RHS = np.zeros(N, dtype=np.float64) #RHS = zeros(len(R), dtype=float)
    for i in range(N): #weave.inline(codeRHS, ['N', 'E', 'l', 'R', 'Veff', 'RHS'],
        RHS[i] = 2.0 * (-E + 0.5 * l * (l + 1) / (R[i] * R[i]) + Veff[i])         #type_converters=weave.converters.blitz, compiler = 'gcc')
    return RHS
@njit
def SolvePoisson(Zq, R, rho):
    ...
    U = np.empty(len(R), dtype=np.float64)
    for i in range(len(R)):
        U[i] = -4.0 * np.pi * R[i] * rho[i]
    Nmax = len(R)
    dx = (R[-1] - R[0]) / (Nmax - 1) #dx = float( (R[-1]-R[0])/(len(R)-1.) )
    Solution = np.zeros(Nmax, dtype=np.float64) #Solution = zeros(len(R), dtype=float)
    
    Solution[0]=0
    Solution[1]=(R[1]-R[0]) # Boundary condition for U_H=V_H/r
    
    h2 = dx * dx #weave.inline(codeNumerovInh, ['U', 'Nmax', 'dx', 'Solution'],
    h12 = h2 / 12.0             #type_converters=weave.converters.blitz, compiler = 'gcc')
    w0 = Solution[0] - h12 * U[0]
    Ux = U[1]
    w1 = Solution[1] - h12 * Ux
    Phi = Solution[1]
    for i in range(2, Nmax):
        w2 = 2.0 * w1 - w0 + h2 * Ux
        w0 = w1
...
def ComputeInterstitialOverlap(Km, RMuffinTin, Vol):
    ...
    for i in range(len(Km)):
        ...
        for j in range(i+1, len(Km)):
            ...
            fbessel = special.spherical_jn(1,KKl*RMuffinTin)      #fbessel = special.sph_jn(1,KKl*RMuffinTin)[0][1]      # j1(|K'-K|S)
...
def Wave(Z, Enu, R0, Veff):
    ...
    for l in range(len(Enu)):
        ...
        ur *= 1/sqrt( integrate.simpson(ur*ur, x = R0) )  #ur *= 1/sqrt( integrate.simps(ur*ur, R0) )  # normalization
        ...
        alpha = integrate.simpson(ur*urp, x = R0) #alpha = integrate.simps(ur*urp, R0)
        ...
        # <\cdot{\psi}|\cdot{\psi}>
        PsipPsip = integrate.simpson(urp*urp, x = R0) #PsipPsip = integrate.simps(urp*urp, R0)
...
def FindCoreStates(core, R, Veff, Z, fraction=4.):
    ...
    for l in range(len(core)):
        ...
        while E<0 and n<core[l]:      # we need ncore[l] bound states
            ...
            if v1*v0<0:
                ...
                norm = abs(integrate.simpson(drho, x = R )) #norm = abs(integrate.simps(drho, R ))
    ...
    print('   Found core states for (n,l) = [', end=' ') #print '   Found core states for (n,l)=[',
    for state in states:
        print('(%d,%d)' % state[:2], end=' ') #print '(%d,%d)' % state[:2],
    print('], E = [', end=' ') #print '] E=[',
    for state in states:
        print('%f,' % state[2], end=' ') #print '%f,' % state[2],
    print(']') #print ']'
...
def ComputeEigensystem(k, Km, Olap_I, Enu, logDer, RMuffinTin, Vol, VKSi=0):
    ...
    N = Km.shape[0] #weave.inline(codeArg, ['Km', 'k', 'qv', 'qvs', 'argums'],
                 #type_converters=weave.converters.blitz, compiler = 'gcc')
    for iK in range(N):
        for i in range(3):
            qv[iK, i] = Km[iK, i] + k[i]
        qvs[iK] = np.sqrt(qv[iK, 0]**2 + qv[iK, 1]**2 + qv[iK, 2]**2)
    for iK in range(N):
        for jK in range(N):
            qvqv = 0.0
            for i in range(3):
                qvqv += qv[iK, i] * qv[jK, i]
            denom = qvs[iK] * qvs[jK]
            if denom == 0.0:
                argums[iK, jK] = 1.0
            else:
                argums[iK, jK] = qvqv / denom
    ...
    for iK in range(argums.shape[0]): #weave.inline(codeLegendre, ['argums', 'lmax', 'Leg'],
        for jK in range(argums.shape[1]):         #type_converters=weave.converters.blitz, compiler = 'gcc')
            x = argums[iK, jK]
            x2 = x * x
            Leg[iK, jK, 0] = 1.0
            if lmax >= 1:
                Leg[iK, jK, 1] = x
            if lmax >= 2:
                Leg[iK, jK, 2] = 1.5 * x2 - 0.5
            if lmax >= 3:
                Leg[iK, jK, 3] = x * (2.5 * x2 - 1.5)
            if lmax >= 4:
                Leg[iK, jK, 4] = 0.375 * (1 - 10 * x2 * (1 - 1.1666666666666667 * x2))
            if lmax >= 5:
                Leg[iK, jK, 5] = 1.875 * x * (1 - 4.66666666666666667 * x2 * (1 - 0.9 * x2))
    ...
    # It computes the Hamiltonian and Overlap in Muffin-Tin and interstitials
    ...
    for iK in range(Ham.shape[0]): #weave.inline(codeHO, ['Olap', 'Ham', 'WK0', 'WK1', 'WK2', 'Enu', 'Leg', 'qvs', 'PP', 'Olap_I', 'VKSi', 'a_lk', 'b_lk', 'C0n'],
        for jK in range(Ham.shape[1]):         #type_converters=weave.converters.blitz, compiler = 'gcc')
            olapMT = 0.0
            hamMT = 0.0
            for l in range(Enu.size):
                Plk = C0n[l] * Leg[iK, jK, l]
...
def rootChemicalPotential(mu, Ek, wkp, Zval, beta=50.):
    ...
    return_val = Zt;
    """'''
    Zt = 0.0 #Zt = weave.inline(code_mu, ['mu', 'Ek', 'beta', 'wkp'],type_converters=weave.converters.blitz, compiler = 'gcc')
    for ik in range(Ek.shape[0]):
        for p in range(Ek.shape[1]):
            x = beta * (Ek[ik, p] - mu)
            if abs(x) < 100.0:
                ferm = 1.0 / (np.exp(x) + 1.0)
            else:
                ferm = 1.0 if x < 0.0 else 0.0
            Zt += wkp[ik] * ferm
    return 2*Zt-Zval
...
def ComputeMTDensity(mu, Ek, wkp, w0, w1, w2, Psi_l, Psip_l, beta=50.):
    ...
    for l in range(nlmax):
        for ik in range(shape(Ek)[0]):
            w0k = w0[ik][l,:] #w0k = array(w0[ik][l,:]) # weight for all bands
            w1k = w1[ik][l,:] #w1k = array(w1[ik][l,:])
            w2k = w2[ik][l,:] #w2k = array(w2[ik][l,:])
            ek = Ek[ik]      #ek = array(Ek[ik])      # eigenvalues for all bands
            #dws = weave.inline(code_sum, ['mu', 'ek', 'beta', 'w0k', 'w1k', 'w2k'],type_converters=weave.converters.blitz, compiler = 'gcc')
            dw0 = 0.0
            dw1 = 0.0
            dw2 = 0.0
            for p in range(ek.shape[0]):
                x = beta * (ek[p] - mu)
                if abs(x) < 100.0:
                    ferm = 1.0 / (np.exp(x) + 1.0)
                else:
                ...
                dw1 += w1k[p] * ferm
                dw2 += w2k[p] * ferm
            wgh[l, 0] += dw0 * wkp[ik] #wgh[l,:] += array(dws) * wkp[ik] # accumulates weight for each l and [psi*psi,dot{psi}*psi,dot{psi}*dot{psi}]
            wgh[l, 1] += dw1 * wkp[ik]
            wgh[l, 2] += dw2 * wkp[ik]
...
def Atom_ChargeDensity(states, R, Veff, Z):
    ...
    for state in states:
        ...
        u2 = u**2
        norm = abs(integrate.simpson(u2, x = R)) #norm = abs(integrate.simps(u2, R))
        u2 *= 1./norm
...
def Atom_charge(Z, core, mix=0.3, RmaxAtom=10., Natom=3001, precision=1e-5, Nitt=100):
   ...
   states.sort(key=functools.cmp_to_key(Atom_cmpb)) #states.sort(Atom_cmpb)
   # Computes charge
   ...
   for itt in range(Nitt):
       ...
       Etot = integrate.simpson(pot, x = R0) + Ebs #Etot = integrate.simps(pot, R0) + Ebs
       Ediff = abs(Etot-Etot_old)
       
       print(RED, '   %d) Etot = %f, Eband = %f, Ediff = %f' % (itt, Etot, Ebs, Ediff), DEFAULT_COLOR) #print '   %d) Etot=%f Eband=%f Ediff=%f' % (itt, Etot, Ebs, Ediff)
...
if __name__ == '__main__':
    ...
    print("Z core =", Zcor, "and Zval =", Zval) #print "Z core=", Zcor, " and Zval=", Zval
    ...
    print("Muffin-Tin radius =", RMuffinTin) #print "Muffin-Tin radius =", RMuffinTin
    print("Volume of the MT sphere    =", VMT) #print "Volume of the MT sphere    =", VMT
    print("Volume of the unit cell    =", fcc.Volume) #print "Volume of the unit cell    =", fcc.Volume
    print("Volume of the interstitial =", Vinter) #print "Volume of the interstitial =", Vinter
    ...
    for itt in range(Nitt):  # self-consistent loop
        ...
        print('%d) Preparing potential' % itt) #print '%d) Preparing potential' % itt
        ...
        nVeff -= zeroMT
        print('   Muffin-Tin zero is', zeroMT) #print '   Muffin-Tin zero is ', zeroMT
        ...
        print('   coreZ =', coreZ, ', coreE =', coreE) #print '   coreZ=', coreZ, 'coreE=', coreE
        ...
        print(GREEN, 'New chemical potential is', mu, DEFAULT_COLOR) #print GREEN, 'New chemical potential is', mu, DEFAULT_COLOR
        
        MTRho = ComputeMTDensity(mu, Ek, fcc.wkp, w0, w1, w2, Psi_l, Psip_l, beta)
        nTotRho = MTRho + coreRho
        
        sMTRho = integrate.simpson(MTRho*R0**2*(4*pi), x = R0) #sMTRho = integrate.simps(MTRho*R0**2*(4*pi), R0)
        sIntRho = ComputeInterstitialCharge(mu, Ek, wi, fcc.wkp, beta)
        sCoreRho = integrate.simpson(coreRho*R0**2*(4*pi), x = R0) #sCoreRho = integrate.simps(coreRho*R0**2*(4*pi), R0)
        
        print('   Zval =', Zval, '~', sMTRho+sIntRho) #print '   Zval=', Zval, '~', sMTRho+sIntRho
        print('   Weight in the MT sphere =', sMTRho, 'and in the interstitials =', sIntRho, 'and in core =', sCoreRho) #print '   Weght in the MT sphere =', sMTRho, 'and in the interstitials =', sIntRho, 'and in core =', sCoreRho
        
        renorm = Z/(sMTRho+sIntRho+sCoreRho)
        print('   Total charge found =', sMTRho+sIntRho+sCoreRho, 'should be', Z, '-> renormalizing by', renorm) #print '   Total charge found=', sMTRho+sIntRho+sCoreRho, 'should be', Z, '-> renormalizing by', renorm
        nTotRho *= renorm

        DiffRho = integrate.simpson(abs(nTotRho-TotRho), x = R0) #DiffRho = integrate.simps(abs(nTotRho-TotRho), R0)
        print(BLUE, 'Electron density difference =', DiffRho, DEFAULT_COLOR) #print BLUE, 'Electron density difference=', DiffRho, DEFAULT_COLOR 
    ...
    savefig('bands')
    show()
    np.savetxt("Ek_raw.dat", Ek)
    with open("Ek_with_kindex.dat", "w") as f:
        for i, row in enumerate(Ek):
            line = f"{i} " + " ".join(f"{e:.6f}" for e in row)
            f.write(line + "\n")
