...
c Input parameters
     ...
     & noapw,l_soc,l_qsgw,lcore_soc,use_sym,bandinfo,otimer
      ...
      real_dp                    :: tolerance
      real_dp                    :: maxmem,mtthr,ene_extra,gcutf,cutzero,deltaex,it_elow
      logical                    :: lomit(3),wrtsuscep,wrtdielec,wrtscreen,wrtinfo,wrtext
...
# endif
      ...
      real_dp                  :: gcut
      real_dp,     allocatable :: bas1(:,:,:,:,:),bas2(:,:,:,:,:),ubas(:,:,:,:),dubas(:,:,:,:),ebas(:,:,:,:)
      integer,     allocatable :: gpt(:,:),pgpt(:,:),ngpt(:),pntgpt(:,:,:)

      ! subspaces / irreps
      complex_dp,  allocatable :: irrep_sub(:,:,:,:)
      integer,     allocatable :: psub(:),block(:,:)
      integer                  :: nsub
