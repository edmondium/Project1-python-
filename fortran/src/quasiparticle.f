          ...
          if(oselfc==2) then
            ...
            if(i==1) then
              if(oselfc==1) then
                ...
                if(any(spec(:n).gt.wspec(1)-spec_warn).or.any(spec(n+1:).lt.wspec(2)+spec_warn))
...
c     Open file to write quasiparticle states
      if(job1%full) then
        ...
        write(iunit) job1%type,size(block,2)
      endif
...
c     Wannier interpolation
      if(l_inter.and.spin<3) then
        Rbegin
        write(6,*)
        call complete_ene(enec(wanbandi:wanbandf,:,s1:s2),wanbandi,wanbandf,spin,2,i,j,k,0,.true.)
        ...
        write(6,*)
        call wannier_interpolation('1',enec(wanbandi:wanbandf,:,s1:s2),spin)
        Rend
        if(wspec(3)/=0) then
...
c                p=2 - use precalculated pade_t(i,j)
      ...
      if(oselfc==1) then
        if     (p==0) then ; call prepare_pade(pd0,1,1,iselfc-1,state,.false.)
...
c     fullt            - true (GT FULL), false (GT)      
      ...
      fullt = job1%full
      if(full.eqv.fullt) then ; allocate(blockt(size(block,1),size(block,2))) ; blockt      = block
...
c     Sets stage for GT definition (mode=0), GT calculation (mode=2), or resets stage for GW (mode=1)
      ...
      if(mode==1) then ; if(.not.associated(pnt)) Bug('pnt not assigned.')
        ...
        frq       =  freq  ; allocate(blk(size(block,1),size(block,2)))
        blk       =  block
...
c     ------------
      ...
      integer                             :: ndeg
      integer                             :: i,j,i1,j1
      if(n/=count(block(:,iblock)/=0)) Error('Count error.')
      i     = block(1,iblock)
      ikptq = job1%kpt(i)
      ispin = job1%spin(i)
      i     = job1%band(i)
...
# endif
     ...
     &  write(6,'(A)') 'Calculate Hermitian self-energy for QSGW with numerical fits. This may take some time.'
      ...
      do iblock = 1,size(block,2)
        n    = count(block(:,iblock)/=0) ; allocate ( self(n,n),vxc(n,n),sxc(n,n) )
...
c     (normally, n=m, unless m=1. Then pade(:n,1) is essentially one-dimensional.)
      subroutine prepare_pade(pade,n,m,iselfc0,state,lwrite)
      implicit none
