...
c       (2) Spin-orbit coupling. Rotation about 360° yields a factor -1 so that the right-hand side should be -identity.
c
...
c     Dummy definition if only diagonal elements are calculated (define a block for each band index in job1%band containing only that band index)
      if(.not.job1%full) then
        allocate ( block(1,size(job1%band)) )
        block(1,:) = [ (i,i=1,size(job1%band)) ]
        return
      endif
      ...
      if(.not.use_sym) then
        do
          ...
          do i = 1,size(job1%band)
            ...
            n  = n + 1
            n1 = max(n,n1)
            if(allocated(block)) block(n,ib) = i
          enddo
          if(allocated(block)) then ; exit
          else                      ; allocate ( block(n1,ib) )
          endif
        enddo
        return
      endif
