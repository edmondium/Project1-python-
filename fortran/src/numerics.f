...
# endif
          ...
          if(x<-0.5) Warn('Large negative exponent x in extrapolation c*r**x: '//Chf(x,'ES9.1')//'  Atom type: '//Chr(itype))
          intgrf = r1*f(1) / (x+1)
        endif
      else
        intgrf = 0
...
c        write(60,*) l,real(matY(1)),real(extra)
        ...
        else if(l==20)            then ; Warn('Inversion not converged with l=20.')
        ...
        l = l + 2
      enddo
      if(l>20) Warn('Inversion converged with l='//chr(l))
 1    continue
# ifdef boost
      if(l>=4) matY(1) = extra
# endif
