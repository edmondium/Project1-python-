...
# ifdef MPI        
        ...
        if(bandinfo) then
          ifR n = size(lines)
          call Mcast(n) ; MnoR( allocate(lines(n)) )
          do i = 1,n
            call Mcast(lines(i))
          enddo
        endif
...
c       Diagonalize Hamiltonian
        ...
        call Mdiagonalize(evec,eig,hamiltonian,overlap,1,nband1,-huge(0d0),maxene)
...
c       Second variation for spin-orbit coupling
     ...
     &                                                     matmul ( cmt4(:,n2) , cmt3 )
          ...
          call Mdiagonalize ( evec_soc,eig,hamiltonian_soc,1,nband_soc,-huge(0d0),maxene)
          Nfence(evec_soc)
...
c
      ...
      call calc_vxc_p(vxchlp,band,nb,ikpt,ispin,l_qsgw)
      vxcmat = unpackmat(vxchlp)
      end

c --------------------------------------

c     Returns vxc matrix in packed storage.
c
      subroutine calc_vxc_p(vxcmat,band,nb,ikpt,ispin,l_qsgw_)
      use readwrite
      use wrapper, only: matvec
      use, intrinsic :: iso_fortran_env
      implicit none
      integer,     intent(in)  :: nb,band(nb),ikpt,ispin
      logical,     intent(in)  :: l_qsgw_
...
# endif
 ...
 1    integ = 0
     ...
     &                                   cmt(lm1:lm2:n1,ic,iband,ikindx,2) * sym(isym)%esoc(2,ispin1)
      ...
      if(l_qsgw_) call read_qsgw(vxcmat,band,nb,ikpt,ispin)
      end
...
c --------------------------------------
      ...
      if(nspin==2) then
        ...
        do isym = 1,nsym
          ...
          sym1(n) = isym
        enddo
        write(iunit,*) n,n,all(sym(sym1(:n))%symmor)
        do isym = 1,n
...
c     Returns list of q vectors in qvec(:,:nq) as obtained from KPTPATH or a file.
c
c begin interface
