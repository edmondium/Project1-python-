...
# endif
     ...
     &                   '      We check this with the shortest, nonzero k-vector.'
      ...
      do k = 1,nkpt1 ; ikpt = kpt1(k)
        if(ikpt==1) call subtract_sphaverage(coulomb1(:,k))
      enddo
...
c     from the fact that MT functions have k-dependent Fourier coefficients (see script).
      subroutine subtract_sphaverage(coulomb1)
      use global
      use wrapper
      use, intrinsic :: iso_fortran_env
      implicit none
      MCOMPLEX_dp, intent(inout) :: coulomb1(dim*(dim+1)/2)
      complex_dp                 :: coeff(nbasm(1)),cderiv(nbasm(1),-1:1),claplace(nbasm(1))
      integer                    :: l,m,i,j,n,itype,ieq
      real_dp                    :: intgrf
      MCOMPLEX_dp                :: stepfunction
c      MCOMPLEX_dp                :: olap(ngptm(1),ngptm(1))
c      call olap_pw ( olap,gptm(:,pgptm(:ngptm(1),1)),ngptm(1) )
...
# endif
      ...
      ! Subtract head contributions from coulomb(:,1) to obtain the body
      l = 0
      do j = 1,n
        do i = 1,j
          l           = l + 1
...
# endif
      ...
      constfunc = coeff / sqrt ( ( sum(abs(coeff(:nbasp))**2) + stepfunction([0,0,0]) ) )!dotprod ( coeff(nbasp+1:), matmul(olap,coeff(nbasp+1:)) ) ) )
...
c     Diagonalize
      ...
      if(nbasm0>=nbasm1) then
        call Mdiagonalize ( eigv,eig,coulmat,olapmat MpiC(lwrite=.true.) )
        nbasm0 = nbasm1
      else if(nbasm0>0) then
        call Mdiagonalize ( eigv,eig,coulmat,olapmat,nbasm1-nbasm0,nbasm1,0d0,0d0 MpiC(lwrite=.true.) )
        nbasm0 = nbasm1
        do while(abs(eig(1)-eig(n1))<1d-8)
          n1 = n1 + 1 ; if(n1>nbasm0) Error('Eigenvalue index out of range.')
        enddo
      else
        call Mdiagonalize ( eigv,eig,coulmat,olapmat,1,nbasm0,x,1d30 MpiC(lwrite=.true.) )
      endif
      Nfence(eigv)
