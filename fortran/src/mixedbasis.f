...
c     Read spex.inp
      ...
      if(maxindx>0) then
        allocate (seleco(maxindx,0:maxlcut,ntype),selecu(maxindx,0:maxlcut,ntype),charr(ntype))
...
c      write(*,*) i,nkpt2,i*nkpt2
 ...
 1    ngptm = 0
      ...
      do ikpt = 1,nkpt2
        do i = 1,ngptm(ikpt)
          work(i) = sqrt(sum(matmul(rlat,gptm(:,pgptm(i,ikpt)))**2)) + i*1d-10 ! "+i*1d-10" prevents rounding errors from changing the order
        enddo
...
c     ----------------
      ...
      ! Preamble
      write(iunit) 0 ! version number (non-positive)
      write(iunit) mode
      ! MB parameters
      write(iunit) ntype
      write(iunit) lat
