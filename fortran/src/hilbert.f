...
c     Define mesh frqs(1:N) for Hilbert trafo (integration over spectral function).
 ...
 1      rdum  = wmin1 + winc * a ; if(fspec(1)<0) rdum = 0 ! Backwards compatibility: FSPEC
        ...
        ! Test Hilbert mesh
        do i = 1,nfrqs-1
          if(frqs(i)<0.or.frqs(i)>=frqs(i+1)) Error('Detected an error in the spectral-function mesh.')
          if(i>1) then
            if(abs(a-(frqs(i)-frqs(i-1))/(frqs(i+1)-frqs(i-1)))>1d-13)
     &                                        Error('Detected an error in the spectral-function mesh.')
          endif
        enddo
      endif
