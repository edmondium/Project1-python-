...
# ifdef test_mtrafo
      real :: rdum1
      Mpi( Error('test_mtrafo not possible with MPI.') )
      allocate ( wing2(maxbasm*(maxbasm+1)/2,3) )
      allocate ( wing(maxbasm,maxbasm,3) )
      allocate ( headc(maxbasm,maxbasm,3) )
      write(*,*) 'Test mtrafo ...'
      do k = 1,nkpt
        write(*,*)
        write(*,*) k,kptp(k),symkpt(k),symkpt(k)>nsymt
        rdum  = 0
        n     = nbasm(k)
        call coulombmatrix(wing2(:,1),nbasm(k),1,k,.false.)
        call coulombmatrix(wing2(:,2),nbasm(k),1,kptp(k),.false.)

        wing(:,:,1) = unpackmat(wing2(:,1)) ; headc(:,:,1) = wing(:,:,1)
        wing(:,:,2) = unpackmat(wing2(:,2)) ; headc(:,:,2) = wing(:,:,2)

        call mtrafo_r(wing(:,:,3),wing(:,:,2),maxbasm,maxbasm,kptp(k),symkpt(k),3,.false.)
        mat(1,1) = sum(wing(:n,:n,1))/sum(wing(:n,:n,3))
        rdum1    = sum(abs(wing(:n,:n,1)-wing(:n,:n,3)*mat(1,1)))
        write(*,*) rdum1,mat(1,1)
        rdum = rdum + rdum1

        call mtrafo(headc(:,:,3),headc(:,:,2),maxbasm,maxbasm,kptp(k),symkpt(k),3,.false.)
        mat(1,1) = sum(headc(:n,:n,1))/sum(headc(:n,:n,3))
        rdum1    = sum(abs(headc(:n,:n,1)-headc(:n,:n,3)*mat(1,1)))
        write(*,*) rdum1,mat(1,1)
        rdum = rdum + rdum1

        call mtrafo_r(wing(:,:,3),wing(:,:,1),maxbasm,maxbasm,kptp(k),-symkpt(k),3,.false.)
        mat(1,1) = sum(wing(:n,:n,2))/sum(wing(:n,:n,3))
        rdum1    = sum(abs(wing(:n,:n,2)-wing(:n,:n,3)*mat(1,1)))
        write(*,*) rdum1,mat(1,1)
        rdum = rdum + rdum1

        call mtrafo(headc(:,:,3),headc(:,:,1),maxbasm,maxbasm,kptp(k),-symkpt(k),3,.false.)
        mat(1,1) = sum(headc(:n,:n,2))/sum(headc(:n,:n,3))
        rdum1    = sum(abs(headc(:n,:n,2)-headc(:n,:n,3)*mat(1,1)))
        write(*,*) rdum1,mat(1,1)
        rdum = rdum + rdum1

        n              = ngptm(k)
        wing (:n,:n,:) = wing (nbasp+1:nbasp+n,nbasp+1:nbasp+n,:)
        headc(:n,:n,:) = headc(nbasp+1:nbasp+n,nbasp+1:nbasp+n,:)

        call mtrafo_pw_r(wing(:,:,3),wing(:,:,2),maxbasm,maxbasm,kptp(k),symkpt(k),3,.false.)
        mat(1,1) = sum(wing(:n,:n,1))/sum(wing(:n,:n,3))
        rdum1    = sum(abs(wing(:n,:n,1)-wing(:n,:n,3)*mat(1,1)))
        write(*,*) rdum1,mat(1,1)
        rdum = rdum + rdum1

        call mtrafo_pw(headc(:,:,3),headc(:,:,2),maxbasm,maxbasm,kptp(k),symkpt(k),3,.false.)
        mat(1,1) = sum(headc(:n,:n,1))/sum(headc(:n,:n,3))
        rdum1    = sum(abs(headc(:n,:n,1)-headc(:n,:n,3)*mat(1,1)))
        write(*,*) rdum1,mat(1,1)
        rdum = rdum + rdum1

        call mtrafo_pw_r(wing(:,:,3),wing(:,:,1),maxbasm,maxbasm,kptp(k),-symkpt(k),3,.false.)
        mat(1,1) = sum(wing(:n,:n,2))/sum(wing(:n,:n,3))
        rdum1    = sum(abs(wing(:n,:n,2)-wing(:n,:n,3)*mat(1,1)))
        write(*,*) rdum1,mat(1,1)
        rdum = rdum + rdum1

        call mtrafo_pw(headc(:,:,3),headc(:,:,1),maxbasm,maxbasm,kptp(k),-symkpt(k),3,.false.)
        mat(1,1) = sum(headc(:n,:n,2))/sum(headc(:n,:n,3))
        rdum1    = sum(abs(headc(:n,:n,2)-headc(:n,:n,3)*mat(1,1)))
        write(*,*) rdum1,mat(1,1)
        rdum = rdum + rdum1
        ...
        rdum = rdum / nbasm(k)**2
        write(*,*) 'SUM',rdum
        if(rdum.gt.1d-12) stop 'Error found. Check!'
      enddo
      stop 'Test done.'
# endif
      ...
      if(any(job1%type==J_SPEC)) then ; n = size(job1%kpt) ; allocate(kpt_list(n)) ; kpt_list = job1%kpt
      else                            ; n = nkpti          ; allocate(kpt_list(n)) ; kpt_list = [(i,i=1,nkpti)]
      endif
...
c     Remove kpoints from kpt_list that have already been calculated
      ...
      n = 0
      do i = 1,m
        if(all(lkpt(i,:))) then ; kpt_list(i:m-1) = kpt_list(i+1:)
        else                    ; n               = n + 1
        endif
      enddo
      call reallocate(kpt_list,n)
...
# ifdef old_mpikpt
      ...
      Msub   = 0
      Mcolor = 0
      n      = count ( [ (any(.not.lkpt(ikpt,:)),ikpt=1,kpt_up) ] )
      if(all(job1%type/=J_SPEC).and.Osize>1.and.n>1) then
...
# define Ifwrt if(n>0 Mpi(.and.Mcolor<=1) )
      j = size(lkpt,1)
      n = count(lkpt(:j,1))
      Rif(iand(restart,W_sigx)/=0 .and. (n<j.and.eval(1)) ) then
              call write_dump(1,[0],                         0,'spex.sigx',job1 MpiC(Mcolor) MpiC(Maxcolor) )
        Ifwrt call write_dump(1,pack([(k,k=1,j)],lkpt(:j,1)),n,'spex.sigx',job1 MpiC(Mcolor) MpiC(Maxcolor) )
      endif
      n = count(lkpt(:j,2))
      Rif(iand(restart,W_sigc)/=0 .and. (n<j.and.eval(2).and.job1%type==J_GW) ) then
              call write_dump(2,[0],                         0,'spex.sigc',job1 MpiC(Mcolor) MpiC(Maxcolor) )
        Ifwrt call write_dump(2,pack([(k,k=1,j)],lkpt(:j,2)),n,'spex.sigc',job1 MpiC(Mcolor) MpiC(Maxcolor) )
      endif
      j = size(kpt_list)
      Rif(iand(restart,W_wcou)/=0 .and. (n<j.and.allocated(barew)) ) then
              call write_dump(3,[0],                         0,'spex.wcou',job1 MpiC(Mcolor) MpiC(Maxcolor) )
        Ifwrt call write_dump(3,pack([(k,k=1,j)],lkpt(:j,2)),n,'spex.wcou',job1 MpiC(Mcolor) MpiC(Maxcolor) )
      endif
      n = count(lkpt(:j,3))
      Rif(iand(restart,W_ccou)/=0 .and. (n<j.and.eval(3)) ) then
              call write_dump(4,[0],                         0,'spex.ccou',job1 MpiC(Mcolor) MpiC(Maxcolor) )
        Ifwrt call write_dump(4,pack([(k,k=1,j)],lkpt(:j,3)),n,'spex.ccou',job1 MpiC(Mcolor) MpiC(Maxcolor) )
      endif
# undef Ifwrt
...
# endif
        ...
        if(kpt_up>1)           cjob = trim(cjob)//'.'//chr(ikpt1,'I3.3')
...
# endif
        Rcall checkmem('matrix/matrixc', MBYTES*NFREQ*n*(n+1)/2 + 16d0*NFREQC*n**2)
        Rcall checkmem('matrix/matrixc',-MBYTES*NFREQ*n*(n+1)/2 - 16d0*NFREQC*n**2)
        Nallocate0 ( matrix, (S_ n*(n+1)/2,NFREQ  S_))
        Nallocate0 ( matrixc,(S_ n,n,      NFREQC S_))
# ifdef gammatest
        if(ikpt==1) then
# endif
        allocate  ( wing(3,n,nfreq),head(6,nfreq) )         ! head and wing not used if ikpt/=1 or spin>=3
        allocate  ( wingc(3,n,2,nfreqc),headc(3,3,nfreqc) ) ! head and wing not used if ikpt/=1 or spin>=3
# ifdef gammatest
        endif
# endif
        ...
        call susceptibility ( ikpt,spin,n,ctrafo,coul,trafo,symon,plasma,nfreqc,freqc,   Win(matrixc),headc,wingc ,
...
c       Transform to Coulomb eigenbasis
 ...
 1      if(wrtsuscep.or.any(job1%type==[J_SUS,J_SUSR])) then
          ...
          Rcall writeout(suscep,suscepc,n,job1%out,'suscep','Susceptibility')
          Nfence(matrix)
          Nfence(matrixc)
...
c       If we treat the Gamma point, enforce suscep to be the body, i.e. set suscep(1,:), suscep(:,1), and wing(:,1) to zero
        if(ikpt==1) then
          ...
          if(nfreq/=0) then
            ...
            j    = 1
            do i = 1,n
              rdum = rdum + sum(abs(suscep(j,:))**2)
              j    = j + i
            enddo
            write(6,'(A,F20.15)')   'Residual head: ',sqrt(sum(abs(wing(:,1,:))**2)/nfreq)
            write(6,'(A,F20.15'NoA) 'Residual wings:',sqrt(rdum/(n*nfreq))
          ...
          if(nfreqc/=0) then
            ...
            write(6,'(A,F20.15'NoA) 'Residual wings:',sqrt((sum(abs(suscepc(:,1,:))**2)+sum(abs(suscepc(1,:,:))**2))/(n*nfreqc))
            if     (metal)                           then ; write(6,'(A)') '  (not small for a metallic system)'
            else if(.not.fullpw.and.any(lcutp<lcut)) then ; write(6,'(A)') '  (small for large GCUT and LCUT in sec. WFPROD)'
            else if(.not.fullpw)                     then ; write(6,'(A)') '  (small for large GCUT)'
            else if(any(lcutp<lcut))                 then ; write(6,'(A)') '  (small for large LCUT in sec. WFPROD)'
...
c       Calculate dielectric matrices (Drude term is added later)
 ...
 2      if(wrtdielec.or.job1%type==J_DIEL) then
          ...
          call writeout(dielec,dielecc,n,job1%out,'dielec','Dielectric matrix')
          if(ikpt==1) then
            dielec   (1,:) = 1
            dielecc(1,1,:) = 1
          endif
...
c       Invert dielectric matrices
        ...
        do i = 1,nfreq ; Pcycle(i,nfreq,.true.)
          beginSplit(i)
          call Minverse(dielec(:,i))
          Mpi( step = Msize )
          endSplit
          PDistr(i,j,dielec(:,j),step)
        enddo
        do i = 1,nfreqc ; Pcycle(i,nfreqc,.true.)
          beginSplit(i)
          call Minverse(dielecc(:,:,i))
          Mpi( step = Msize )
          endSplit
          PDistr(i,j,dielecc(:,:,j),step)
        enddo
        Rcall cpu_done(cputime)
...
c          call teststop('new SUSCEPR')
          ...
          Nfence(matrix)
          Nfence(matrixc)
          Rcall writeout(suscep,suscepc,n,job1%out,'suscepR','Renormalized susceptibility matrix')
          Nfence(matrix)
          Nfence(matrixc)
          nullify(suscep)
          nullify(suscepc)
...
c       Calculate screened interaction
        ...
        if(wrtscreen.or.job1%type==J_SCR) then
          ...
          Rcall writeout(screen,screenc,n,job1%out,'screen','Screened potential')
          Rif(ikpt==1) then
            screen   (1,:) = 0
            screenc(1,1,:) = 0
          endif
        ...
        if(job1%type==J_SCRW) then
          ...
          call cpu_time(cputime)
          call coulomb_wannier(screenw,barew,screenc,headc,ctrafo,n,ikpt,nfreqc,rsite,nsite,spinw)
...
c       Update self-energy
        Nfence(matrix)
        Nfence(matrixc)
        if(any(eval(:2))) call selfenergy(job1,ikpt,eval,n,coul,Win(ctrafo),Win(matrix), head, wing,
...
c     ------------
      ...
      implicit none
      character(80), allocatable :: charr(:)
...
c     Determines basis set - defines n, ctrafo, coul, trafo, symon, (gpw)
      ...
      if(optm<=1) then
        ...
        !   allocation
        allocate  ( coul(n) )
        Nallocate0( ctrafo, (S_ n,n S_) )
        Nallocate ( olap,   (S_ ngptm(ikpt),ngptm(ikpt) S_) )

        !   diagonalize Coulomb matrix
        if(optm==1) n = max(0,nint(optcutm))
        call diagonalize_coulomb(Win(ctrafo),coul,Win(olap),coulomb0,ikpt,n,-optcutm)
        Ndeallocate ( olap )
        if(optm==0.and.n/=nbasm(ikpt)) then
          if(noapw) then ; Error('Numbers of Coulomb eigenvectors and MPB functions disagree.')
          else           ; RInfo('MPB reduced by APW construction. Trafo done in susceptibility.')
          endif
          trafo = .true.
        endif
        leng = 5+max(2,len_trim(chr(ceiling(coul(n)))))
        Rwrite(6,'(A,F'//chr(leng)//'.4,''   (MB'',F6.2,'')'')') 'Minimum Coulomb eigenvalue:',coul(n),sqrt(4*pi/coul(n))
        call reallocate ( coul,n )
        ...
        n = max(0,nint(optcutm))
        allocate ( gpw(ngptm(ikpt)) )
        call getgpw(gpw,n,-optcutm)

        !   determine transformation coefficients
        call getctrafo(Win(ctrafo),n,gpw,ikpt)
...
c     ------------
      ...
      subroutine getgpw(gpw,n,optcutm_r)
      implicit none
      ...
      rarr = rarr(gpw)
      if(ngptm(ikpt)==1) then ; n = 1 ; return ; endif
      if(n==0) then
        n = 1
        do while(4*pi/rarr(n+1)**2>optcutm_r)
          n = n + 1 ; if(n==ngptm(ikpt)) exit
        enddo
      else
        do while(n<ngptm(ikpt))
          if(abs(rarr(n+1)-rarr(n))>1d-8) exit
          n = n + 1
        enddo
      endif
...
# define ERRSTOP if(Herr/=0) Error('Fatal HDF5 error.')
     ...
     &        Chr(real(freqc(nfreqc)))//'+img*'//Chr(imag(freqc(nfreqc)))
      ...
      Nallocate0 ( ctrafo,  (S_ nbasm(ikpt),n   S_) )
      Nallocate0 ( matrix,  (S_ n*(n+1)/2,NFREQ S_) )
      Nallocate0 ( matrixc, (S_ n,n,NFREQC      S_) )
      allocate   ( coul(n) )
      allocate   ( head(6,nfreq)     ) ; allocate ( wing(3,n,nfreq) )
      allocate   ( headc(3,3,nfreqc) ) ; allocate ( wingc(3,n,2,nfreqc) )
      ! Read data
      ...
      if(nfreq>0) then
        call hdf_rdwr(Hkpt,'matrix',0,matrix) ; Nfence(matrix)
        if(ikpt==1) then
      ...
      if(nfreqc>0) then
        call hdf_rdwr(Hkpt,'matrixc',0,matrixc) ; Nfence(matrixc)
        if(ikpt==1) then
      ...
      if(typ==1) then
        ...
        dielec => matrix ; dielecc => matrixc ; allocate(sqcoul(n)) ; sqcoul = sqrt(coul)
...
# endif
      ...
      ! Write data
      call hdf_rdwr(Hkpt,'coul',  1,coul(:n))
      call hdf_rdwr(Hkpt,'ctrafo',1,ctrafo(:,:n))
      ...
      if(nfreqc>0) then
        call hdf_rdwr(Hkpt,'matrixc',1,matrixc)
        if(ikpt==1) then
...
#   define CA call Mcastl
      ...
      implicit none
      CB(plasma); CB(n)   ; CB(coul); CB(sqcoul); CB(ctrafo)
...
c     MPI: Called only by root process(es).
      ...
      complex_dp               :: matc(maxlmindxm,maxlmindxm,ncent) InvC(ctrafo1(nbasp,n))
      real                     :: cputime
      integer                  :: g(3)
      ...
      call cpu_time(cputime)
# ifdef INV
      ctrafo1 = ctrafo(:nbasp,:n)
      call desymmetrize(ctrafo1,nbasp,n,1)
# else
...
# endif
      ...
      do ifreq = 1,nfreq
        ...
        do itype = 1,ntype
          ...
          do ieq = 1,neq(itype)
            ic                   = ic + 1
            matc(:nlmp,:nlmp,ic) = matmac ( matmat ( CTRAFO1(m+1:m+nlmp,:n) , screen(:,ifreq) ) -
     &                                      matdiag( CTRAFO1(m+1:m+nlmp,:n) , coul            ) ,
     &                                               CTRAFO1(m+1:m+nlmp,:n) )
            m                    = m + nlmp
          enddo
        enddo
      ...
      if(job1%type==J_COSX) then
        ...
        call unitarytrafo( mat(:,:,1) , screen(:,1) - diagonalp(coul) , ctrafo(nbasp+1:,:n) , 2 )
        do k = 1,nkpt
          if(kptp(k)==ikpt) then
            call mtrafo_pw_r(mat(:,:,2),mat(:,:,1),ngptm(ikpt),ngptm(ikpt),ikpt,symkpt(k),3,.false.)
            do j = 1,ngptm(k)
              do i = 1,ngptm(k)
                g                          = gptm(:,pgptm(i,k)) - gptm(:,pgptm(j,k))
