...
c
c     If <keyword> is not found, the program stops with an error message
c     unless one of <default> and <status> is given.
c     If <default> is given, then <dest>:=<default>.
c     If <status>  is given, then <status> takes one of the values
...
c     The line length is restricted to 256 characters.
...
c     A leading '#' denotes a comment.
      ...
      integer,       parameter  :: nkey = 100
      character(8),  parameter  :: key(nkey) = [
     &                             'BZ      ','NBAND   ','CHKOLAP ','CHKMISM ','WRTKPT  ',
     &                             'KPT     ','GAUSS   ','JOB     ','WRITE   ','MEM     ',
     &                             'RESTART ','STOREIBZ','ALIGNBD ','CORES   ','BANDINFO',
     &                             'DIPOLE  ','ITERATE ','BANDOMIT','KPTPATH ','MTACCUR ',
     &                             'CORESOC ','IBC     ','PLUSSOC ','ENERGY  ','BLOECHL ',
     &                             'CUTZERO ','MPIKPT  ','MPISPLIT','DELTAEX ','NOSYM   ',
     &                             'TRSOFF  ','STOREBZ ','TIMING  ',
     &                             'SECTION ','MBASIS  ','TOL     ','SELECT  ',
     &                                        'LCUT    ','GCUT    ','CHKPROD ','WFADJUST',
     &                                        'OPTIMIZE','NOAPW   ','ADDBAS  ',
     &                             'SECTION ','WFPROD  ','MTTHR   ','MINCPW  ','APPROXPW',
     &                                        'LCUT    ','FFT     ',
     &                             'SECTION ','COULOMB ','LEXP    ','TSTCOUL ','NOSTORE ',
     &                                        'STEPRAD ','CHKCOUL ',
     &                             'SECTION ','SUSCEP  ','HILBERT ','TETRAF  ','MULTDIFF',
     &                                        'DISORDER','HUBBARD ','WGHTTHR ','PLASMA  ',
     &                                        'FSPEC   ','FPADE   ',
     &                             'SECTION ','SENERGY ','MESH    ','FREQINT ','ZERO    ',
     &                                        'CONTINUE','CONTOUR ','ALIGNVXC','VXC     ',
     &                                        'MPIBLK  ','MPISYM  ','SPECTRAL','ORDER   ',
     &                                        'SMOOTH  ','LOGDIV  ',
     &                             'SECTION ','WANNIER ','UREAD   ','WSCALE  ','RSITE   ',
     &                                        'ORBITALS','MAXIMIZE','FROZEN  ','DISENTGL',
     &                                        'BACKFOLD','IRREP   ','CUTGOLD ','INTERPOL',
     &                                        'SUBSET  ','WBLOCH  ','PLOT    ' ]
