...
# include "cppmacro.h"
      ...
      character(22), parameter      :: dftlong    = 'FLEUR v0.26b (2019.03)'
      character(5),  parameter      :: dftshort   = 'fleur'
      character(6),  parameter      :: file_wavef = 'KS.hdf'
...
c ----------------------------------
      ...
      integer              :: iunit
      integer              :: i,l,idum,l1,l2,l3
      ! Read gwa : LAPW parameters
      iunit = fopen('gwa',form='unformatted')
      read(iunit) i
      idum = -2
      if(i/=idum) then
        write(0,'(A,I3,A,I3)') 'read_LAPWparam: Fleur-GW version is',-i,', but I need version',-idum
        Error(' ')
      endif
