...
#   ifdef MPI
     ...
     &  '_CHECKMEM',add,mem0/megabyte,total,real(stk)/kilobyte,mem_prv0/megabyte,private,trim(arraystring)//trim(chr_nalloc)
#   else
     ...
     &  'CHECKMEM',add,mem/megabyte,total,real(stk)/kilobyte,trim(arraystring)//trim(chr_nalloc)
#   endif
# endif
      if(addmem/=0) then
        if(mem>maxmem) then
